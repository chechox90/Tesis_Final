                ...
                using (SolusegEntities context = new SolusegEntities())
                {
                    List<DTO_HorarioConductorMostrar> dtoHorario = (from hrcon in context.HORARIO_CONDUCTOR
                                                                    ...
                                                                    select new DTO_HorarioConductorMostrar
                                                                    {
                                                                        ...
                                                                        APELLIDO_MATERNO = usr.APELLIDO_MATERNO,
                                                                        ESTADO = usr.ESTADO,
                                                                        HORARIO_REGISTRADO = context.REGISTRO_HORARIO.Where(x => x.ID_HORARIO == hrcon.ID_HORARIO).FirstOrDefault() == null ? 0 : 1,
                                                                        ID_TERMINAL = hrcon.ID_TERMINAL_INICIO,
                                                                        NUMERO_JORNADA = hrcon.NUMERO_JORNADA,
                                                                        FECHA_HORA_INICIO = hrcon.FECHA_INICIO,
                                                                        TIPO_CONTRATO = tic.NOMBRE_TIPO_CONTRATO,
        ...
        public int SetCambiarEstadoCubierto(int idTurno)
        {
            try
            {
                ...
                using (SolusegEntities context = new SolusegEntities())
                {
                    using (var contextTransaction = context.Database.BeginTransaction())
                    {
                        bool estado = context.HORARIO_CONDUCTOR.Where(x => x.ESTADO == true && x.ID_HORARIO == idTurno).FirstOrDefault().HORARIO_CUBIERTO;
                        ...
                        {
                            HORARIO_CONDUCTOR horarioOLD = context.HORARIO_CONDUCTOR.Where(x => x.ESTADO == true && x.ID_HORARIO == idTurno).FirstOrDefault();
                            horarioOLD.HORARIO_CUBIERTO = true;
                            respuesta = context.SaveChanges();
